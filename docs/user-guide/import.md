...
Scene-linear EXR does not receive a metered exposure adjustment. Its generated SDR rendition starts at zero Exposure with fixed middle-gray placement and Peak Fit/Smooth color rolloff. If an imported gain-map image contains an authored SDR base, that display rendition is used directly and SDR Highlight Compression starts bypassed to avoid adding another shoulder.
...
This is not universal DNG or camera compatibility. The current local sample matrix proves structural routing and successful decode; producer-reference comparisons and macOS resource validation must finish before color, shading, geometry, or DJI visual compatibility can be signed off.
