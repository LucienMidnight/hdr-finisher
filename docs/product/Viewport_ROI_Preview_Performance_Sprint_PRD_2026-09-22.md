...
| Phase 4 | **Complete (implementation and development gates); packaged/owner acceptance carried into Phase 5** | Adaptive coarse rendering, display-scale exact refinement, response preferences, migration, diagnostics and the normal UI replacement are recorded in §15.36. Browser and unit evidence covers the exit gates on the development workstation. A review at the owner's request (§15.40) found no Phase 4 response-logic defect: the remaining latency deficit is the native source stream and cache churn, which Phase 5 items 1–4 own. Packaged Electron latency, the owner's image, pan latency and cross-hardware cold-cache behavior are now Phase 5 acceptance items rather than open-ended debt. |
| Phase 5 | **Complete (workstation evidence); owner items disclosed (§15.40–§15.56)** | Items 1 (central allocator with mandatory registration, global LRU and pinned reservations; 0.0% peak-agreement drift at 42 MP), 2 (Auto calibrated from verified device information plus a bounded probe, policy ceiling and stated fallback), 3 (bounded staging ring, one queue drain per stream), 4 (chunked `/proxy-stream` selected on measurement: −50.7% transport stage, −49% native arrival; compression/socket/shared-memory rejections measured), 5 (inactive lane deferred to true idle), 6 (device-loss rebuild ladder, corruption recovery, six-cycle endurance flat at 282 MB with zero evictions) and 7 (hardware-matrix harness, log and recorded rows) are complete. Close-out: the Electron runner was repaired (host `ELECTRON_RUN_AS_NODE`, a shadow review launcher) and the packaged baseline captured at the §8 reference viewport 2560 × 1440 (§15.47–§15.48); the integrated adapter row was captured on the hybrid workstation (`amd / rdna-2`, 0.0% drift) with the CPU-only row and the adverse 4K-display-on-integrated case disclosed untested (§15.51); §8.4 coverage was audited with 400% zoom and 1/4/8 locals closed and two negative-control self-checks added, one control explicitly deferred (§15.49); §8.1 headline latency was reported under the input-to-compositor rule (§15.50) — current-pixel, pan, cold first visible and handler pass warm, refined fails at 289–296 ms p95 against the <100 ms target, caused by the native source arrival. Remaining owner/decision items: a CPU-only machine row, the adverse 4K-on-integrated row, and the progressive-fill decision the refined-target gap names. |
...
The normal tier selector removal left older standalone browser and Electron drivers querying `#preview-resolution`. Drivers that intentionally compare 1K/2K/4K/Full now set the Settings-only diagnostic override. The UI interaction, startup, focus and menu drivers now exercise the normal Responsive/Balanced/Precise control. The Full-tier browser driver also enables the explicit Exact peak scope option before requiring `peak_exact`; its prior unconditional assertion timed out with that option off, after the CPU Full render itself had succeeded. On the development server at port 8799, the startup-state, preview-response/native-zoom, and GPU/CPU Full diagnostic browser drivers passed. The development Electron confirmation/focus driver passed after the control change; the menu driver and packaged build were not exercised. This maintenance does not change the Phase 4 packaged latency or visual stability acceptance status.

### 15.38 Owner field feedback and Phase 4 regression triage — 2026-09-23

**Owner observation on the development Electron build.** Zooming out can show source preparation at `0/3` channels for a long, variable time. Zooming in sometimes says it is preparing a native exact view, but sometimes leaves a blurry view with no visible work status until it resolves. In Balanced, adjustments at zoom feel about as laggy as the former Full setting. Responsive feels similar; all three response choices feel much the same and laggy at high zoom. The previous 1K/2K/4K/Full experience felt substantially better. Treat this as a product regression report, not as acceptance of the Phase 4 UI. The exact image, zoom sequence, hardware timing, and cache state have not yet been captured, so durations and root cause remain unmeasured.

**Code-inspection findings, not a reproduction claim.** `scheduleZoomRefinement()` waits 80 ms and dispatches the required exact edge directly for every response preference. The latency controller is used by the interactive adjustment scheduler but not this zoom path; thus Responsive, Balanced and Precise are expected to behave alike for zoom-scale changes. `viewerState()` decides Ready from the stored `accepted.exact`, generation and geometry, but does not compare the accepted processing edge with the current `requiredProcessingLongEdge()`. Zoom changes do not increment the edit generation. A formerly exact frame can therefore continue to report Ready while CSS displays it at a new scale and exact rendering is pending. This is a concrete status-contract defect that plausibly explains silent blur.

Source levels are keyed by the exact requested long edge. A cold level performs three sequential Lanczos channel resizes; progress stays at `0/3` until the first channel completes. The default persistent mip budgets are 256 MiB memory and 1 GiB disk, while the browser GPU retains at most two proxy levels per lane. Different zoom-out percentages can request different levels, and large levels may miss memory or later be evicted. These are plausible sources of variable waits, but cache-hit/eviction and source-size evidence from the owner's case is still needed. The controller fits render wall time to a quadratic pixel-cost model even when that time includes source loading, denoise preparation or other fixed work; this may distort scale selection and should be checked with stage timing rather than assumed as the cause.

**Next action before Phase 5.** Reproduce the owner's sequence on the current development Electron build and, if available, a freshly packaged build. Capture event-to-first-current-pixel and event-to-exact-pixel timings, accepted generation/edge, visible status, source-mip hit/build/eviction counters, transport route, graph, and GPU/backend stages for cold and warm repeats at Fit, zoom-out steps, 100%, 200%, a pan, and representative adjustments under all three preferences. Compare with commit `55455e6` and its manual tiers on the same fixture and hardware. Fix the zoom status contract and make the response choice affect zoom feedback; then address measured source-build stalls or cache churn. Add regression tests for silent blur/Ready and cold-vs-warm zoom behavior. Re-run ROI/export parity gates if render scheduling or scale changes. The Phase 5 allocator and transport work may improve measured bottlenecks, but this Phase 4 regression should not be deferred to Phase 5 without restoring the owner's acceptable interaction experience.

### 15.39 Phase 4 regression repair and measured development evidence — 2026-09-23

**Reproduction boundary.** Two attempts to start the Electron test runner, including one with isolated user data, failed before a test page loaded with DevTools WebSocket `ECONNRESET`; the owner's already-running Electron process was left alone. No owner image was identified. The reproducible evidence below uses the deterministic 7968×5320 noisy TIFF (42.4 MP), and a 4200×2800 variant, in Edge/WebGPU on the same Windows workstation. An isolated checkout of Phase 3 `55455e6` and the current checkout used separate servers and mip-cache roots. The drivers are `tests/performance/phase3-manual-baseline.js` and `tests/performance/phase4-regression-browser.js`; captured data is in `output/performance/phase3-manual-baseline-42mp.json`, `phase3-manual-adjustments-4200.json`, `phase4-regression-final-42mp.json`, and `phase4-regression-final-4200.json`. These are sequential local runs, not a randomized benchmark or packaged Electron acceptance result.

**Fault confirmed.** Before repair, the 4200 px Responsive cold 50% zoom took 2032 ms to first current pixels and 2036 ms to exact pixels while the old 800 px frame immediately said `Ready — Display exact`. The zoom callback requested exact directly under every preference. A clean 42 MP cache run had 2 cold builds and 1757 ms of source-build time during the first 50% step (1816 ms first current, 3737 ms exact); the next 35% step caused 2 more builds, and even a return to 50% caused another build. The three-channel Lanczos source preparation and exact-edge cache keys therefore explain a material part of the variable `0/3` wait. A cache-warm run still showed a 1368 ms tiled proxy request at native edge, with about 56 ms to its first source tile, so source upload/GPU queue work also remains material. This measurement does not isolate every millisecond of the owner's Electron stall.

**Repair.** Viewer readiness now also requires the accepted processing edge to equal the edge requested by the current zoom; an old-scale frame immediately says `Preparing view` until current coarse or exact pixels arrive. Zoom invokes the same response controller as edit interaction: Responsive can show a 1024 px coarse view, Balanced a 2048 px coarse view when useful, and Precise sends exact. Coarse presentations remain explicitly labeled, and every current zoom still schedules exact refinement. The coarse edge is stable across neighboring zoom percentages to reuse a source level. Zoom cancels stale scheduled edit/inactive-lane preparation before foreground work; inactive source preparation receives an abort signal and generation guard. The accepted tiled execution path also allows a bounded coarse adjustment frame after an edit-generation bump. No backend cache budget or resize algorithm was changed: cold construction remains costly, and exact native upload remains a measured limitation.

| 42 MP sequence, cache-warm final run | Responsive first current / exact | Balanced first current / exact | Precise first current = exact |
| --- | ---: | ---: | ---: |
| 50% zoom | 113 / 1906 ms | 570 / 574 ms | 475 / 471 ms |
| 35% zoom | 97 / 308 ms | 182 / 342 ms | 259 / 260 ms |
| 100% native | 113 / 1616 ms | 97 / 1483 ms | 1447 / 1444 ms |
| warm 50% return | 107 / 489 ms | 195 / 514 ms | 418 / 418 ms |
| warm 100% return | 97 / 1588 ms | 93 / 1556 ms | 1505 / 1503 ms |
| Exposure edit at native | 65 / 231 ms | exact 311 ms | exact 298 ms |

The 42 MP final run logged 10 memory evictions at the first 50% step but no new cold builds in its warm return; the 4200 px final rerun logged zero cold builds at both 50% steps, with 4 memory evictions at the first step and none on return. Its Responsive native zoom was 125 ms to current and 575 ms to exact; Balanced was 212/668 ms and Precise 565/568 ms. At 42 MP native exact, WebGPU used streamed/tiled transport: the stage trace reports a 1368 ms source proxy request, 21 source chunks, 45 tile submissions, and about 14.7 MB tiled working set. The source-mip counters and stage trace should be read together; a mip memory hit does not imply a cheap GPU upload. The 200% step did not change the required native processing edge, so no new scale render was owed; Fit completed at roughly 100 ms. Pan-specific timing was not captured by this driver.

**Phase 3 comparison.** On the same 42 MP fixture, the `55455e6` manual 2K tier's 50% step was 55 ms when its 2048 px source was already resident, and its subsequent native 100% step was 314 ms; 4K and Full native steps were 1537 and 261 ms respectively in their sequential order. On the 4200 px fixture, manual 2K and Full native Exposure edits both measured 293 ms. These tier runs inherit prior cache/renderer state and do not measure the first visible frame separately. They support the owner's report that a resident low tier can feel quicker; they do not establish a clean cold superiority of Phase 3. The repaired Responsive path restores early current pixels while preserving the exact native obligation.

**Gates and remaining acceptance.** The new status unit case, coarse-edge unit cases and browser driver assert old-scale `Ready` is impossible, Responsive presents coarse, Precise does not, warm 50% reuses its coarse edge without a cold build, and all zooms converge to the required edge. `phase4-preview.js` passed, including native 2400 px inspection and byte-equal coarse/exact comparison. The 300% Denoise/film/spatial ROI sweep was byte equal at three 52,866-pixel regions (`maxAbsDifference` 0); export parity completed across its cases. Full suites: Node 207 passed; Python 1368 passed, 3 skipped. Packaged Electron latency, the owner's image, pan latency, and cold-cache behavior across hardware remain open product-acceptance work before Phase 5 is declared ready.

### 15.40 Phase 4 review, baseline capture, and Phase 5 first increment — 2026-09-24

**Phase 4 review outcome.** Reviewed the Phase 4 implementation at `2f6050d` at the owner's request ("functions work, performance not great"). All six exit gates have development evidence; the focused Phase 4/5 units and the full suites re-ran clean (Node 208 passed; Python 1369 passed, 3 skipped). The remaining deficit is source-upload and cache-churn cost, not response logic: the 42 MP native obligation is a whole-frame 323 MiB stream (21 chunks) whose proxy request dominates exact arrival, and the first 50% step still evicts memory levels. Phase 4 is therefore closed as implementation-and-development-gates complete; packaged Electron latency, the owner's image, pan latency and cross-hardware cold-cache behavior move into Phase 5 acceptance (per §15.38–15.39) instead of staying open-ended.

**Baseline capture.** `phase5-baseline-4200.json` (pre-change) and `phase5-final-42mp.json` (post-change) capture Fit / 50 / 35 / 100 / 200 / warm returns / exposure / detail for all three preferences, plus pan. The driver now records pan (retained-frame luma, settle, viewer status) and the scheduler snapshot per step; pan had never been captured. Pan stays painted (luma 0.38–0.44) and settles inside the driver's 350 ms sample window. Packaged Electron remains uncaptured: the runner's DevTools WebSocket failed before page startup in §15.39, and this checkpoint uses the same Edge/WebGPU workstation boundary.

**Phase 5 item 3 — staged upload ring.** `loadProxyStreamed` and `loadProxyRegion` now write row chunks through a bounded ring (`SOURCE_UPLOAD_RING_SIZE` 4 × 16 MiB) whose only reuse fence is `mapAsync` on that slot's own earlier copy, and drain the queue once per stream instead of once per chunk. Unit evidence: `source-transport.test.js` asserts four slots carry a 32-chunk stream with one queue drain (the prior code drained per chunk) and every staging buffer is released. Interleaved same-session A/B at 42 MP against the reconstructed per-chunk drain: ring 1387 / 1439 / 1443 ms vs drains 1598 / 1671 / 2218 ms for the 21-chunk, 323 MiB native stream (−22% mean, tighter spread); first-tile time unchanged (65–78 ms). At 4200 px the full-driver A/B measured the proxy request 656→430 ms and 560→401 ms (Responsive / Balanced) and native-100 exact 803→560 and 879→653 ms. The full 42 MP driver runs measured that native proxy stage at 1440–1521 ms; a driver run is not a controlled benchmark and carries the same system noise the A/B above controls for.

**Phase 5 item 5 — inactive lane deferred to true idle.** `scheduleInactive` re-asks a `canRun` predicate when the idle callback fires and retries after 400 ms rather than loading the other lane's whole-frame proxy behind foreground work; `metrics.inactiveDeferred` counts it. Explicit comparison intent is no longer gated by readiness: the single-frame control stays enabled, a click starts the lane at foreground priority before `switchLane`, and a held compare on an unprepared lane starts the preload and completes the peek when it lands if the hold is still down. Browser evidence: `tests/performance/phase5-inactive-lane.js` (`npm run test:phase5-inactive-lane`) asserts a busy editor defers (deferral counted, gate consulted) and that an unprepared held compare presents the peek only after the explicit preload lands, then releases cleanly. `test:comparison-disclosure` still passes.

**Carry-over — cold build channels.** `downsample_image` resizes a source's channels concurrently on a 3-thread pool above 2 MP (Pillow releases the GIL for this resize); small and single-channel sources stay sequential, so the progress/cancellation contract in `test_render_cache` is unchanged. Measured component A/B on a 42 MP channel stack: 618 ms → 280 ms (2.21×). `test_core.py` pins bit-identical output and monotone progress against the sequential path.

**Carry-over — stage-aware controller timing: checked and rejected.** Feeding the controller the renderer's graph-only stage time (source and mask waits excluded) instead of total wall time made Responsive skip the coarse frame at warm 50% at 42 MP — the driver gate failed with "Responsive zoom never presented coarse pixels" / "Warm zoom changed the reusable coarse source level" — because a scale change always pays a fresh source upload, and that upload is exactly the cost the coarse frame buys down. Total wall time stays the controller's input; the attempted stage split was reverted and the finding is recorded here instead of shipped.

**Remaining Phase 5 work.** Items 1 (mandatory central reservations, global LRU), 2 (Auto budget calibration), 4 (transport selection with measured copies and latency), 6 (device-loss rebuild, corruption recovery, endurance), 7 (supported hardware matrix). Given these measurements the recommended order is item 4 and item 1 first — the 42 MP native arrival is dominated by the whole-frame stream and by evictions at the first 50% step — then item 2, item 6, and the hardware matrix in item 7.

### 15.41 Phase 5 item 4 — transport selection and streaming route — 2026-09-24

**What was measured.** Two meters, both on the 7968 × 5320 deterministic noisy TIFF: a wire probe (`tests/performance/phase5-transport-probe.js`, Node, no GPU; evidence `output/performance/phase5-transport-probe.json`, three interleaved repeats) and an in-browser A/B (`tests/performance/phase5-transport-ab.js`, `npm run test:phase5-transport-ab`; evidence `output/performance/phase5-transport-ab.json`). Each A/B run clears the renderer (`resetSession`), parks a Fit frame, switches to Precise and zooms to 100%, so every candidate fetches and uploads the whole native frame from a cold renderer over a warm backend mip. All candidates transferred exactly 339,118,080 bytes (323.4 MiB).

| candidate | wire probe, median | transport stage, median | first row | native arrival wall |
|---|---|---|---|---|
| 21 sequential `/source-tile` strips, per-chunk requests | 894 ms (ttfb 523, body 367) | 1260 ms | 60 ms | 1442 ms |
| 4-way concurrent strips | 2038 ms (ttfb 1000, body 946) | not run | — | — |
| one prebuilt `/proxy` body | 823 ms (ttfb 462, body 372) | 781 ms | 505 ms | 902 ms |
| one chunked `/proxy-stream` body | not run | 621 ms | 86 ms | 732 ms |

Concurrent strip requests regress 2.3× (backend encodes serialise), so request parallelism is not a lever. The prebuilt body reaches first byte only after the whole 339 MB is encoded (462 ms ttfb), which is why its first row lands at 505 ms. The measured win is the streaming route: **−50.7% transport stage (1260 → 621 ms) and −49% native arrival (1442 → 732 ms)**, with first row at 86 ms instead of 60 ms-then-serialised.

**Selection.** One authenticated same-origin HTTP response per frame (`GET /api/session/{id}/proxy-stream/{kind}`), read as chunked 16 MiB row strips through the existing bounded staging ring. The transport stays inside the existing packaging/security boundary: no new scheme, port, or path permission; the Electron token injection, CSP `connect-src 'self'` and the 401 boundary apply unchanged. Rejected with evidence:

- **Compression / frontend decode.** Ratios are real (native body: gzip 2.35×, brotli 3.19×; 7952-resampled body: gzip 1.63×) but the backend cost is not: Node zlib gzip level 6 took 6.19 s and brotli q4 6.10 s for one 42 MP frame. That is an order of magnitude over the selected route's whole 621 ms stage, and on the streaming route the wire is already hidden behind the encode, so compression cannot pay for itself.
- **Binary socket.** The probe shows per-request overhead is not the bottleneck (21 requests 894 ms vs 1 request 823 ms) and concurrency regresses; a WebSocket would still carry the same bytes and the same backend encode, while bypassing the Electron `webRequest` token injection that authenticates `/api/*` today, so it would need a new authenticated channel and paired packaging work for no measured gain.
- **Shared memory / file-backed.** Neither reduces bytes or backend encode; the Electron renderer would need Node access or path grants that the current sandbox and desktop-path-grant model deliberately avoid (R7), and the browser (development) surface cannot test the path at all.

**Why the frame is still whole-frame.** A region would be ~35 MB instead of 339 MB (measured earlier at native in `roi-source-transport.js`: 23.3% of the frame for a 4096 ROI), but at a scale change the presentation target is new and presents the whole processed frame to the canvas, so every tile must be painted before the frame is presentable — there is no retained same-scale frame to anchor `loadProxyRegion`. Region-first-native therefore needs a progressive-fill design (upscaled retained backdrop, visible tiles at native, catch-up fills the rest), which is a product-visible semantics change, not a transport swap. It is deferred as the next candidate if native arrival must go below the ~620 ms backend-encode floor; the conceptual next safe edit is to present the retained coarse frame as the new target's backdrop and let the existing deferred catch-up own the rest.

**Landed changes.** Backend: `rgba_proxy_pixel_format` and `encode_rgba_proxy_rows` (row slices are byte-identical to the whole-frame encode; the half-float decision is resolved against the whole frame), `PROXY_STREAM_CHUNK_BYTES` 16 MiB, and `GET /api/session/{id}/proxy-stream/{kind}` returning a `StreamingResponse` of row strips with the same headers and pixel bytes as `/proxy`; a superseded edit ends the body early, which the client treats as a recoverable short stream. Frontend: `loadProxyStreaming` consumes `response.body.getReader()` (or a one-piece body) into the bounded ring with the same row math as the strip route; `sourceTransportMode` defaults to `"stream"` and keeps `"single"` and `"strips"` for the A/B driver and field fallback; `window.HDRFinisherPerformance.setSourceTransport` exposes it. Unit evidence: six new cases in `tests/source-transport.test.js` (one request, row coverage in order, ring of four, one drain, bounded JS buffer, short-stream recoverability, unavailable-route fallback, superseded mid-stream stop); Python: row-strip byte parity against `encode_rgba_proxy` in `tests/test_render_cache.py` and a `/proxy-stream` versus `/proxy` byte/header equality case in `tests/test_api.py`.

**Gate check.** `tests/performance/phase4-regression-browser.js --width 7968 --height 5320` (evidence `output/performance/phase5-transport-regression-42mp.json`) passes every Phase 4 gate with the streaming default: Responsive presents coarse, warm 50% reuses the coarse edge with zero cold builds, Precise never presents coarse, every zoom reaches its required edge, pan stays painted (luma 0.38–0.44). Native-100 exact is now 891 / 857 / 943 ms (Responsive / Balanced / Precise) and warm-100 is 830 / 800 / 781 ms, against 1616 / 1483 / 1444 ms and 1588 / 1556 / 1588 ms in the §15.39 table. Full suites: **Node 214 passed; Python 1370 passed, 3 skipped.**

**Remaining Phase 5 work.** Items 2 (Auto budget calibration), 6 (device-loss rebuild, corruption recovery, endurance), 7 (supported hardware matrix plus packaged Electron baseline). Next safe edit: item 2 — calibrate Auto from verified device information plus a safe allocation probe, keep the stated 2 GiB policy as the conservative fallback, and do not raise Auto before item 6's failure recovery passes.

### 15.42 Phase 5 item 1 — central GPU reservations and global LRU — 2026-09-24

**What landed.** One allocator (`frontend/gpu-allocator.js`, loaded before the renderer) now owns the budget, the LRU order and the reservations for everything the renderer caches:

- **Mandatory registration.** The five cache maps (`proxies`, `sceneLuminance`, `localMasks`, `detailBandTiles`, `maskTiles`) are wrapped by `trackGpuCache`, so a `set`, `delete` or `clear` *is* an allocation event: the entry is registered with its real `byteSize` and an `evict` callback that releases the texture through its own cache, and a `get` touches the global LRU. No call site can forget to report an allocation, including the four proxy load paths and every mask/luminance path.
- **Global LRU eviction.** `enforceBudget` evicts least-recently-touched unpinned entries across all kinds until the incoming allocation fits, then reports what remains unfunded in `overBudgetBytes`. Per-cache caps are gone as the primary policy: a lane now keeps its source levels up to a last-resort cap of six, and the central budget is what actually reclaims them.
- **Reservations.** The tile graph and the retained presentation target are registered pinned; the source proxy a pass reads is pinned for that pass (`renderTiledTo` and `renderTo`), so an allocation inside a pass can never evict the texture being copied from. `reserve()` also tracks working sets that are about to be held before their textures exist.
- **Replacement safety.** A same-key replacement in a tracked cache releases the old texture, not just the registry entry; the instrumented driver caught the old behaviour as unreferenced live memory.

**Why the first 50% step showed evictions.** The per-lane cap was two source levels, so the step's exact level evicted the Fit level and every warm return rebuilt or re-coarsened. Retaining levels turns the measured warm path into a level reuse: on the 42 MP fixture, warm 50% now presents the exact 3984-edge frame in **88 ms with zero cold builds** (cold 50% is 1587 ms and still presents coarse first). The Phase 4 driver's warm assertion was sharpened to the invariant it always meant: warm must not rebuild any source mip, and if it presents coarse at all that coarse must be the cold step's edge — presenting exact immediately is allowed and is what the retained level now buys. Evidence `output/performance/phase5-allocator-regression-42mp.json`; every other Phase 4 gate unchanged.

**Agreement fix.** The retained presentation target was live device memory but missing from the logical resident categories — the peak-agreement driver measured it as 47 MB of drift at 42 MP native. It is now `presentationSurfaceBytes` in the model, and the retained overlap reserves one extra target while a scale change replaces it (the former intermediate-sized proxy for that reserve stays as the fallback when no target exists).

**Exit-gate evidence.** `tests/performance/phase5-peak-agreement.js` (42 MP, `output/performance/phase5-peak-agreement-42mp.json`) wraps the page's `GPUDevice` prototypes before any application script, counts every texture and buffer the app asks the device to create at its real size, subtracts every destroy, and compares the renderer's logical peak with the accounted live bytes at seven settled steps. Result: **worst drift 0.0%; logical equals accounted live at every step** (19 / 514 / 405 / 929 / 1102 / 1102 / 776 MB), unreferenced live memory 0.0–0.1 MB, `overBudgetBytes` 0, evictions 0, and every texture format known to the instrumented ledger. Reported separately, as the gate requires: transition peaks of coexisting graphs while a scale is replaced (+11 to +325 MB across steps) and live bytes whose lifetime is owned outside the registry. An earlier run's 101.5 MB of unreferenced memory at the 50% step was traced to the item-5 inactive-lane preload (the other lane fetching the same edge at true idle) and is now settled and attributed before sampling, never silently ignored.

**Suites.** Node **225 passed** (8 allocator cases, 3 new renderer-wiring cases); Python **1370 passed, 3 skipped**; Phase 4 driver green at 42 MP.

### 15.43 Phase 5 item 2 — Auto budget calibration and its safe fallback — 2026-09-24

**What landed.** `frontend/gpu-budget.js` calibrates Auto once the device exists, from verified device information plus a bounded allocation probe, and the renderer adopts the result (`calibrateGpuBudget`, `memoryBudgetBytes`, the allocator budget, and the diagnostics budget block). The rules follow PRD 5.7 and R6 literally:

- **Never a VRAM claim.** Nothing reads physical memory. The adapter's `isFallbackAdapter` flag and its description, the device's own limits, and a real allocation are the only inputs, and none of them is treated as capacity.
- **The policy is a ceiling.** Auto can only be lowered: a software adapter (verified flag or SwiftShader/llvmpipe/lavapipe description) takes 512 MiB, a device whose limits fall below the reference graph (maxTextureDimension2D < 8192 or maxBufferSize < 128 MiB) takes 1 GiB, and everything else keeps the stated 2 GiB. Auto is not raised above policy before item 6's failure recovery passes.
- **The probe is real and bounded.** A buffer of `min(candidate/2, 256 MiB, maxBufferSize)` is created and destroyed; a failure downgrades one step (`probe-downgrade`, or `probe-floor` on an already-low candidate) and records the probe bytes and outcome rather than throwing.
- **The fallback is stated, not inferred.** With nothing verified, Auto is the 2 GiB policy with `source: "policy-fallback"`, `fallback: true`; with no policy at all it reports `unavailable` and zero.

**Evidence.** Unit: 8 calibration cases in `tests/gpu-budget.test.js` (fallback, software adapter, limited device, probe pass/fail/floor, policy ceiling, no-policy) plus 3 renderer-wiring cases in `tests/webgpu-allocation-agreement.test.js` (calibration adopted by the allocator and diagnostics; an explicit user setting outranks calibration and returning to Auto restores it; a failed probe downgrades before any plan). In-browser: `output/performance/phase5-budget-calibration.json` records the calibration the real Edge/Vulkan session used while also re-running the item 1 agreement gate — adapter NVIDIA with `fallback: false`, a 256 MiB probe passed, Auto = 2 147 483 648 B from `policy-fallback`, and the agreement gate still 0.0% drift at every step.

**Remaining Phase 5 work.** Item 6 (device-loss rebuild, cache corruption recovery, endurance) and item 7 (supported hardware matrix plus the owner's packaged Electron check).

### 15.44 Phase 5 item 6 — device-loss rebuild ladder, cache corruption, endurance — 2026-09-24

**Device loss.** `tests/device-loss-fallback.js` was rewritten around the ladder the application actually implements, and now passes end to end. The previous version waited for the CPU preview after a destroyed device; the automatic in-session rebuild made that expectation unreachable, which is why it failed rather than the application. The new driver asserts:

- two consecutive losses each rebuild the device **in-session** — `available` returns, a *different* device object presents, the WebGPU canvas is back, the transport is not the CPU preview, an exposure edit after each rebuild still lands on the GPU, and the attempt budget is reset by each success (`state.gpuRebuildAttempts === 0`, which is the consecutive-failure policy the application has);
- a rebuild that cannot initialize (the driver forces the initialization seam to fail) is classified, the session keeps working on the authoritative CPU preview (`Raw RGBA8` after the next edit, `/preview-raw/hdr` 200, CPU canvas swaps in), and the loss is named in the display detail;
- a reload restores a fresh WebGPU device; zero browser errors.

**Cache corruption.** The store already validates magic, format version, dtype, channels, dimensions, payload length and CRC32 before serving, discards a bad level and rebuilds it, writes atomically through a temp file, sweeps stale format versions and interrupted temps, and counts every discard (`corrupt_discards`) and write failure. `tests/test_render_cache.py::test_source_mip_corruption_is_discarded_and_rebuilt` covers a flipped byte and a truncated file with byte-parity to the cold build; the sweep test covers stale versions and interrupted writes. Nothing was missing here; the endurance driver now also asserts the counters stay at zero in normal operation, so a false discard would be caught.

**Endurance.** New `tests/performance/phase5-endurance.js` (`npm run test:phase5-endurance`), evidence `output/performance/phase5-endurance-4200.json`. Six cycles of `fit → 50% → 100% → 35%` plus an exposure edit at 4200 × 2800 with the device instrumented from page start. Per-cycle assertions: logical peak within 15% of accounted live memory, unreferenced live memory ≤ max(4 MB, 1%), allocator `overBudgetBytes` 0, peak within the admitted budget (or a classified recovery — none was needed), no allocation backoff, no allocation-failure stage, no CPU fallback, no unknown texture format, source-mip memory and disk within their byte budgets, and `corrupt_discards`/`write_failures` at zero. Result across all six cycles: **live 282 MB constant, drift 0.0% every cycle, orphans 0.1 MB, evictions 0, creates 69→151 and destroys 38→120 (balanced 15 per cycle), cycle 6 = cycle 3 live bytes (0.0% growth)**.

**Suites.** Node **225 passed** (plus the rewritten browser driver), Python **1370 passed, 3 skipped**.

**Remaining Phase 5 work.** Item 7 (supported hardware matrix plus the owner's packaged Electron check).

### 15.45 Phase 5 item 7 — supported hardware matrix — 2026-09-24

**What landed.** `docs/product/Phase5_Hardware_Matrix.md` is the matrix contract and its log; `tests/performance/hardware-matrix-capture.js` (`npm run test:hardware-matrix`) captures one machine's row, and `tests/performance/hardware-matrix-summary.js` merges captured rows into the markdown table. A row records only what the machine measured — WebGPU availability, adapter identity and bounds, the calibrated Auto budget and its source and probe, the 4200 px `fit → 50% → 100% → warm-50 → warm-100` timings, one painted-pan check, the worst peak-agreement drift, and a CPU fallback check. The operator passes the class (`discrete|integrated|unified|cpu-only`) and it is recorded verbatim; nothing is inferred from adapter strings, and a machine with no WebGPU is captured as `captured-cpu-only` through the authoritative CPU preview instead of failing. Physical VRAM is never recorded, because WebGPU never reports it.

**Recorded so far.** `output/performance/hardware/workstation-nvidia.json`: `workstation-nvidia, discrete, WebGPU yes, nvidia/lovelace, software no, Auto 2.00 GiB policy-fallback, native-100 435 ms, warm-50 95 ms, drift 0.0%, CPU fallback not exercised, captured`. The integrated/unified row, the CPU-only row and the **packaged Electron baseline** are explicitly `untested — owner run pending` in the log; they are disclosed as untested rather than inferred from the workstation result.

**Remaining Phase 5 work (owner-side).** Run `test:hardware-matrix` on an integrated/unified machine and on a CPU-only machine, and capture the packaged Electron baselines with the packaged build (`tests/performance/packaged-baselines.js`, `tests/run-in-electron.js`); paste the summary rows into the matrix log. No code work remains for items 1–7 in the development tree.

### 15.46 Phase 5 close-out — final verification after items 1–7

Final state of the development tree with every Phase 5 item implemented:

- **Item 1** — central allocator, mandatory registration, global LRU, pinned reservations; **0.0% peak drift** at all seven 42 MP steps (`output/performance/phase5-peak-agreement-final-42mp.json`; earlier `phase5-peak-agreement-42mp.json`).
- **Item 2** — Auto calibration from verified device information plus a bounded allocation probe, policy ceiling and stated fallback; in-browser NVIDIA row is `policy-fallback`, 2 GiB, 256 MiB probe passed (`phase5-budget-calibration.json`).
- **Item 3** — bounded staging ring, one drain per stream (§15.40).
- **Item 4** — chunked `/proxy-stream` selected on measurement: −50.7% transport stage (1260 → 621 ms), −49% native arrival (1442 → 732 ms), first row 86 ms; compression, binary socket and shared-memory/file-backed rejected with measured or reasoned evidence (§15.41).
- **Item 5** — inactive lane deferred to true idle with explicit comparison intent (§15.40).
- **Item 6** — device-loss rebuild ladder exercised (two in-session rebuilds, then a classified CPU fallback), corruption recovery already store-tested and counter-asserted, endurance flat at 282 MB live across six cycles with 0.0% drift, zero evictions, no failures (`phase5-endurance-4200.json`).
- **Item 7** — matrix harness, log and one recorded row; remaining machine classes and the packaged Electron baseline explicitly untested and owner-run.

**Final suites.** `node --test`: **236 passed**; `pytest tests -q`: **1370 passed, 3 skipped**. `phase4-regression-browser.js --width 7968 --height 5320` (evidence `output/performance/phase5-final-regression-42mp.json`) passes every gate: Responsive presents coarse, warm 50% reuses the retained level (exact in 92 ms, zero cold builds), Precise never presents coarse, every zoom reaches its required edge, pan stays painted (luma 0.38–0.44). 42 MP native-100 exact: 1011 / 165 / 1055 ms (Responsive / Balanced / Precise).

**Exit-gate mapping.** Peak agreement within 15% — 0.0% (§15.42/15.46). No test exceeds the admitted budget without a classified recovery — endurance and peak drivers assert `overBudgetBytes 0` and peak ≤ budget (§15.44). Foreground preempts background work — item 5 deferral plus the endurance driver's zero CPU fallbacks, zero refused drafts and zero allocation backoff (§15.40, §15.44). Selected transport materially reduces copies or latency and passes packaging/security — streaming selection with the rejections measured, security boundary unchanged (§15.41); the packaged verification is the owner's remaining check. Hardware results recorded — §15.45 with the untested classes disclosed.

### 15.47 Phase 5 close-out — the Electron runner and the packaged build — 2026-09-24

The packaged evidence was blocked by two defects outside the application code, both now fixed in `tests/run-in-electron.js`:

- **The host environment ran Electron as Node.** The runner inherited `ELECTRON_RUN_AS_NODE=1` from the shell that started it. Electron then starts as plain Node: `require("electron")` returns the package path, `desktop/main.js` throws on `app.isPackaged` before any window exists, and Playwright either never sees the debugger lines or the DevTools socket resets when the crash kills the process — the ECONNRESET §15.39 recorded. The child is launched with that variable removed.
- **A shadow application.** `desktop/node_modules/electron/dist/resources/app` carried a launcher left by the 2026-08-30 interactive review (`hdr-finisher-interactive-review-launcher`). Electron loaded it instead of the app argument; the launcher sets `HDR_FINISHER_USER_DATA_DIR` to `output/electron-profile-phase1-review-interactive` and then requires the repository's `desktop/main.js`, so every Electron run silently used the review profile and ignored the runner's environment. The runner now passes the absolute `desktop` directory as the app path, and the launcher directory is renamed `app.interactive-review-launcher.disabled` (reversible, untracked `node_modules` state).

Runner additions: a disposable `userData` profile per run (fresh startup state, never the owner's profile), shutdown anchored to the Electron main-process pid (the launcher pid is a shell on Windows), `--packaged` to launch `dist-electron/win-unpacked`, `HDR_FINISHER_ELECTRON_WINDOW_SIZE` seeded as `window-state.json` (a runtime `setContentSize` does not stick and was measured to stall runs), a no-op resize guard in the `newPage` stub, and `HDR_FINISHER_POWER_MODE` from `powerMonitor` for §8 reports. `packaged-baselines.js` and the new latency driver take `--viewport` and record the viewport that actually measured. Verification: `node tests/run-in-electron.js tests/startup-state.js` passes with a fresh profile (defaults `balanced`). The packaged build is PyInstaller backend + `npm run pack:dir`: 0.8.12, Chrome 150.0.7871.224, Electron 43.4.0.

### 15.48 Packaged Electron baselines at the §8 reference setup — 2026-09-24

`node tests/run-in-electron.js tests/performance/packaged-baselines.js --packaged --viewport 2560x1440` → `output/performance/phase5-packaged-baselines-42mp.json`. Packaged app, **2560 × 1440** viewport, 7968 × 5320 source, tiled execution, AC power, zero page errors.

| state | slider→frame | →current | →stable | processed edge | source requests | source bytes |
|---|---|---|---|---|---|---|
| Fit | 5.6 ms | 57 ms | 199.5 ms | 1562 | 1 | 5 603 328 |
| 100% | — | 1035.4 ms | 1035.4 ms | 7968 (native) | 2 | 243 231 |
| 200% | — | 210.4 ms | 210.4 ms | 7968 (native) | 1 | 121 285 |
| 400% | — | 244.5 ms | 244.5 ms | 7968 (native) | 1 | 121 946 |

Pan at 200%: painted (centre luma 0.815), `ready`, accepted generation equals current. The 400% state was added this close-out to close the §8.4 zoom row. Cold/warm, stated rather than implied: Fit is the cold step (one 5.6 MB proxy fetch); the 100% state fetches only 243 KB to present the native 7968 edge, so the native mip came from the backend disk cache and 100/200/400 are warm for the source cache. These are the driver's in-page wall-clock numbers (the §15.20 instrument) and are the packaged reference baselines; they are **not** §8.1 headline claims, which are §15.50.

### 15.49 §8.4 benchmark-matrix coverage and negative controls — 2026-09-24

Coverage of every §8.4 required run, with the driver that carries it:

| §8.4 row | Coverage | Where |
|---|---|---|
| Fit / 100% / 200% / 400% | covered | §15.48 (400% added to `packaged-baselines.js`); `phase4-regression-browser.js`; `native-inspection-dpr.js` |
| cold and warm source cache | covered | Fit cold vs warm states in §15.48; `roi-source-transport.js` (cold builds, single-flight, reuse); `phase4-regression-browser.js` warm assertions |
| pointwise grade | covered | exposure/contrast edits in every driver; `roi-parity.js` pointwise against the reference |
| maximum Detail radius | covered | `roi-parity.js`, `tiled-direct-parity.js` run clarity 3% / sharpen 3 px, the declared maxima; `webgpu-shader-compilation.js` runs 0.3 and 3 |
| Denoise | covered | `denoise-tiled-parity.js`, `denoise-selector-seam.js`, `full-tier-denoise.js` |
| film spatial effects | covered | `film-look-interaction.js`, `tier-film-consistency.js`, `tiled-film-parity.js` |
| one / four / eight locals | covered this close-out | 1/4/8 added to `many-local-layers.js`; run `output/performance/phase5-many-local-layers.json`: submission p95 1.7 / 1.3 / 1.5 ms, `interactionMaskRequests` 0 at every tier (16/32/64 kept as pressure tiers; 64 locals 33 MB, inside the 96 MB assertion) |
| mask grading vs mask geometry change | covered | `tests/test_local_adjustments.py::test_preview_cache_reuses_compiled_mask_when_only_grade_changes`; `gpu-local-adjustments.js` (opacity edits produce zero mask requests, feather edits produce them) |
| exact peak on/off + explicit exact analysis | covered | `scope-exact-peak.js` (default off, panel on/off, explicit native measurement, scope isolation) |
| rapid reversal | covered | `viewer-state-transitions.test.js` (rapid reversal returns to Ready with no intermediate state) |
| tier-preference migration | covered | `preview-preferences-migration.test.js` (all four legacy tiers, no tier override enabled) |
| device loss | covered | `tests/device-loss-fallback.js` (§15.44) |
| transient transport error | covered | `failure-taxonomy.js` injects "Failed to fetch" and asserts the device stays up and a frame paints |
| allocation pressure | covered | `phase5-peak-agreement.js` (0.0% at seven 42 MP steps); `phase5-endurance.js` (six cycles flat) |
| corrupt disk-cache entry | covered | `test_render_cache.py::test_source_mip_corruption_is_discarded_and_rebuilt` plus the sweep tests |
| discrete / integrated / CPU-only machines | discrete and integrated covered; CPU-only untested | §15.51 and the matrix log; no CPU-only machine was available and no class is inferred from a forced software mode |
| standard / high DPI | covered | `native-inspection-dpr.js` dpr 1 and 2 (2400-edge source: processed 2400, backing = css × dpr, viewport true) |
| adverse 4K display on integrated graphics | untested | the workstation display is 2560 × 1440 (QHD) and no integrated-only machine exists; disclosed rather than inferred |

Negative controls — the tests that must fail when the defect is injected:

| Control | Status | Detector |
|---|---|---|
| 4K-at-100% upsample | added this close-out | `native-inspection-dpr.js` factors the native contract and asserts the injected counterfactual (`processedLongEdge: 1024` over a 2400 source) is rejected; the live dpr 1/2 run passed with the counterfactual in place |
| stale-generation presentation | covered | `presentation-gate.js` (a superseded same-size generation must not blank or resize the accepted frame); `viewer-state-transitions.test.js` |
| tile-boundary seam | covered by construction | `tiled-direct-parity.js` compares Direct and Tiled pixel-for-pixel (worst channel 0), so a seam is a difference; `tiled-film-parity.js`, `denoise-selector-seam.js` |
| exact-peak job competing with foreground work | **deferred** | The peak pass skips the presentation gate (`measureOnly`), takes masks through `backgroundMaskRequestCoordinator` rather than the foreground coordinator, and `render-coordinator.test.js` covers the generic priority contract — but no driver runs an exact-peak measurement concurrently with a foreground render. Hook named for the deferred driver: `HDRFinisherPerformance.measureExactPeak()` |
| duplicate cold mask compilation | covered | `test_api.py` (one compile for many tiles, stale-request rejection); `test_local_adjustments.py` (grade-only change reuses the compiled mask); `test_frontend_contract.py` (batched transport); `many-local-layers.js` (no mask regeneration at any tier) |
| canvas clear before replacement | added this close-out | `tier-change-blank-canvas.js` asserts `transitionBlank === 0`; the driver now classifies a synthetic zero-luma frame as blank as a built-in sensitivity check; `presentation-gate.js` blank checks |
| untracked GPU allocation | covered | `webgpu-allocation-agreement.test.js` (recording device stub; every cache map registers through the allocator; eviction through release); `phase5-peak-agreement.js` (0.0% drift) |

### 15.50 §8.1 headline latency under the §8 measurement rule — 2026-09-24

New driver `tests/performance/headline-latency.js`; packaged run at the reference setup: `node tests/run-in-electron.js tests/performance/headline-latency.js --packaged --viewport 2560x1440 --samples 8` → `output/performance/phase5-headline-latency-packaged.json`.

**Instrument.** A capture-phase listener records the trusted input's `event.timeStamp`; a `requestAnimationFrame` loop samples the presented canvas through a 16 × 16 downscale and records the live preview generation, the accepted presentation, exactness and viewer status at every vsync; the application's own `hdrfinisher:preview-presented` event is recorded in the same loop. "Current pixel" is the first sampled frame belonging to a presentation strictly newer than the generation at arm time with a painted canvas; "refined" is the first such frame the accepted presentation reports exact. Observation quantisation is at most one display interval; the report states that bound rather than folding it into the numbers.

| §8.1 metric | target | state | samples | median | p95 | worst | verdict |
|---|---|---|---|---|---|---|---|
| slider → current pixel, Fit, Balanced | < 50 ms | warm | 7 | 22.8 | 23.1 | 210.7 | pass |
| slider → current pixel, Fit, Balanced | < 50 ms | cold | 1 | 222.2 | 222.2 | 222.2 | fail |
| slider → refined, Fit, Balanced | < 100 ms | warm | 7 | 283.7 | 289.5 | 301.5 | fail |
| slider → refined, Fit, Balanced | < 100 ms | cold | 1 | 288.8 | 288.8 | 288.8 | fail |
| slider → refined, 100% zoom | < 100 ms | warm | 7 | 295.6 | 296.1 | 302.1 | fail |
| slider → refined, 100% zoom | < 100 ms | cold | 1 | 452.1 | 452.1 | 452.1 | fail |
| slider → current pixel, 200% zoom | < 50 ms | warm | 7 | 23.3 | 23.6 | 29.4 | pass |
| warm pan → current pixels | < 33 ms | warm | 7 | 11.0 | 11.1 | 11.2 | pass |
| first visible after cold view change (Fit → Actual) | < 150 ms | cold | 1 | 86.3 | 86.3 | 86.3 | pass |
| first visible after view change (warm) | < 150 ms | warm | 7 | 87.0 | 87.3 | 90.1 | pass |
| UI input handler duration | < 16.7 ms | warm | 12 | 6.4 | 7.1 | 7.3 | pass |

Environment, as reported by the run: packaged `hdr-finisher-desktop/0.8.12`, Chrome 150.0.7871.224, Electron 43.4.0, Windows 10.0.26200 x64, adapter `nvidia / lovelace` with `fallback: false`, 2560 × 1440 viewport at dpr 1, 7968 × 5320 source, Balanced preference, AC power. The `view-change → Fit` row has no §8.1 target and reported no sample (the detector did not see a newer-generation or changed-pixel frame within its window); it is recorded as not measured rather than as a pass.

**What the numbers say (measured, not inferred).** The coarse current-pixel path, warm pan, cold first visible frame and input handling are inside their §8.1 targets when warm. The refined (exact) presentation is ~2.9× over the <100 ms target at Fit and 100%, and the cold first stroke is over the 50 ms current-pixel target. The dominant term is the native source arrival measured at the same setup in §15.48 (100% wall-clock 1035 ms; §15.41 measured native arrival at 732 ms on the development surface), not input handling (6.4 ms) or presentation observation. This checkpoint does not claim the §8.1 refined gate; it reports it failed, with the number that fails, so the next work item is the native-arrival path (the deferred progressive-fill decision named in §15.41), not another scheduler change.

### 15.51 Definition-of-done sweep and gate ledger — 2026-09-24

- **Packaged Electron baseline** — captured and recorded at the §8 reference setup (§15.48, `phase5-packaged-baselines-42mp.json`); the runner defect that blocked it is fixed (§15.47); the matrix log row is updated.
- **Hardware matrix** — discrete (existing) and integrated (`output/performance/hardware/workstation-amd-igpu.json`: `amd / rdna-2`, WebGPU yes, 2.00 GiB policy-fallback with the 256 MiB probe passed, native-100 569 ms, warm-50 95 ms, worst step drift 0.014% at native-100 and 0.0% elsewhere, pan painted, no CPU fallback, `captured`). The integrated row is the integrated adapter of the hybrid workstation, pinned with `--gpu low-power`; the operator reported the class and the captured adapter identity is recorded as measured. **CPU-only** and the **adverse 4K-display-on-integrated** case are disclosed untested: no CPU-only machine and no 4K display were available.
- **§8.4 coverage and negative controls** — §15.49; two gaps closed in code (400% zoom in the packaged baseline, 1/4/8 locals) and two detectors given sensitivity self-checks (upsample counterfactual, blank-frame classifier); the exact-peak-versus-foreground control is explicitly deferred with its hook named.
- **§8.1 latency under the measurement rule** — §15.50; current-pixel, pan, cold first visible and handler pass warm; refined fails at 289–296 ms p95 against <100 ms, with the native arrival named as the cause.
- **Export bytes and metadata** — the full Python export suites pass (`test_export_parity.py`, `test_preview_export_parity.py`, `test_ultrahdr_export.py`, `test_sdr_export.py`, `test_export_denoise_parity.py`). On the packaged HDR surface, `tests/run-in-electron.js tests/performance/export-parity.js --packaged` → `output/performance/phase5-export-parity-packaged.json`: six scenarios, every `exportCorrectness` check `ok`, SHA-256 matching the declared digest, `rgba16float` real precision, the 1000-nit ceiling active, zero page errors. Phase 5 changed no export code (items 1–6 are preview allocator, transport and recovery), so identical project state exports the same bytes.
- **Suites** — `node --test`: **236 passed**; `pytest tests -q`: **1370 passed, 3 skipped**.

Gate ledger for the close-out: peak agreement within 15% at 42 MP — **pass** (0.0%). No over-budget work without a classified recovery — **pass**. Foreground preempts background — **pass**. Transport reduces copies/latency with the packaging/security boundary unchanged — **pass** (streaming selection; rejections measured). Hardware results recorded with unsupported configurations disclosed — **pass with disclosures**. §8.1 current-pixel / pan / cold-first-visible / handler targets — **pass warm**. §8.1 refined <100 ms — **fail** (289–296 ms p95 warm). §8.2 export bytes/metadata unchanged — **pass**. §8.4 matrix coverage and negative controls — **pass with one explicit deferral**. Packaged app identity and export precision — **pass**.

### 15.52 Ledger closure — 2026-09-24

Phase 5 is closed in the ledger with the phase-table row updated. No undocumented release blocker remains. The open items are all named and owner- or decision-bound rather than hidden: a CPU-only machine row, the adverse 4K-display-on-integrated row, the deferred exact-peak-versus-foreground negative control, and the §8.1 refined-target gap whose cause is the native source arrival (the progressive-fill decision deferred in §15.41). Evidence files from this close-out: `phase5-packaged-baselines-42mp.json`, `phase5-headline-latency-packaged.json`, `phase5-many-local-layers.json`, `phase5-export-parity-packaged.json`, `hardware/workstation-amd-igpu.json`; the failed-attempt artifacts (`headline-latency-smoke.json`) stay as run history and are not part of the reference set.

### 15.53 Owner-visible stall while adding a luma local — diagnosis and safety net — 2026-09-24

Symptom (packaged app, owner): after adding a luma-range local at 183% zoom, the preview stayed on its coarse proxy with the dock reading "Preparing view — Native region exact". Nothing was running: the preview callbacks had stopped, the mask coordinator had no run for the current generation, the renderer held six idle sockets to the backend, a `py-spy` dump showed every backend thread idle in the threadpool queue, and the app used no CPU. Touching any control recovered it, which is what made it look intermittent.

Reproduction: part 1 of `tests/zoom-refinement-stall.js` fires the same burst (luma local, zoom 200%, grade, zoom 183%) through the packaged app. The measured state: 4 inputs scheduled, 3 frames rendered, nothing queued or in flight; the accepted presentation was a coarse 2048 frame against a required edge of 7968; the mask coordinator's last generation was the 2048 one; zero pending requests; zero page errors. Cause class: the burst retires the scheduler (`previewScheduler.cancel()` from the zoom refinement and from the structural mask gesture), and when the follow-up that retired it is itself superseded or refused without scheduling anything else, the viewer waits forever. Any single later input healed it — the probes that ran the same sequence without a burst never reproduced it.

Fix: a bounded re-arm watchdog (`installPreviewWatchdog()` in `frontend/app.js`; 1 s interval, at most 20 consecutive attempts). While the viewer is not ready and nothing is legitimately in flight — no gesture, no GPU draft, no geometry transaction, no open mask draft, no comparison peek, no render-coordinator lane in flight or pending — it schedules the current generation again; any ordinary schedule resets the attempt budget. `HDRFinisherPerformance.previewWatchdog()` reports its state and `previewWatchdogEnable(bool)` is the diagnostic hook the control uses. This is a safety net over the lost wake-up path, not a removal of it: the zoom refinement still relies on its own follow-up, and the guarantee to the owner is convergence instead of a silent forever-wait.

Control: part 2 of the driver injects a dropped wake-up (a tier change whose single follow-up is cancelled with nothing queued) and requires both halves: with the watchdog disabled the injected state must not converge (measured: still preparing after 6.1 s), and with it enabled the viewer must converge on its own (measured: 1.5 s, one re-arm, ready and exact at the required edge). Evidence: `output/performance/zoom-refinement-stall-dev.json` (development tree: burst 2.57 s with 1 re-arm; disabled 6.09 s no convergence; recovery 1.51 s) and `output/performance/zoom-refinement-stall-packaged.json` (rebuilt PyInstaller bundle + `pack:dir`: burst 2.57 s; disabled 6.15 s no convergence; recovery 1.52 s; zero page errors on both). `node --test` remains 236 pass with the change, and the owner's app was relaunched from the rebuilt package.

### 15.54 Transient partial page load in one owner session — 2026-09-24

After the §15.53 relaunch the owner reported that the local-adjustments mask-tool icons were missing and that Luma followed by `+` created nothing. Neither reproduced anywhere it could be measured: the rebuilt `win-unpacked` app and the installed 0.8.12 both render the icons (the mask image resolves, the `::before` box is 20 × 20, the SVG answers 200 with its 507 bytes) and create the luma local (`.local-adjustment-item` count 0 → 1, selection `luminance_range`, zero console or page errors), both in a fresh profile and in the owner's own profile. The affected window's backend was serving the current stylesheet byte-for-byte (218 460 bytes) and the SVG, and no JavaScript path rebuilds that tool strip, so the failure was confined to that window's runtime page state rather than the build, the profile or the assets. A relaunch cleared it, which the owner confirmed; static markup missing its stylesheet rules together with late-bound handlers being inert is what an incomplete page load looks like. The diagnostic that settled it is recorded for the next such report: launch the packaged app with a loopback `--remote-debugging-port`, attach a recorder for console/page errors and failed requests, and inspect the live page directly. The app currently runs with that diagnostic port open.

### 15.55 Abandoned source streams exhaust the connection pool — diagnosis and fix — 2026-09-24

Symptom (owner, live packaged session): after zooming in and out a few times the viewer stuck at "Preparing view — Display exact"; clicks and panels still worked, the sliders did nothing, and zooming into the 40–60% range was where it started taking ages before hanging.

Live capture (the §15.54 diagnostic port made the stuck window inspectable). The viewer was `updating/scale` with the accepted exact frame at 3646 and the required edge at 4543; `gpuDraftInFlight` held a render coordinator `inFlight` refinement token with a settled request pending behind it. The renderer, the GPU process and the backend each measured **0 CPU** over an 8 s sample, `py-spy` showed every backend thread idle with no handler running, and the uvicorn config uses the default 5 s keep-alive — yet six established connections to the backend HTTP port were 6–9 minutes old. A cheap `fetch('/api/session/{id}')` issued from the live page **did not complete within 4 s**, while the same request answered immediately from outside the app: the renderer's per-origin connection pool was exhausted.

Cause: `loadProxyStreaming` (the §15.41 chunked transport) consumes the response with `reader.read()` and checks currency *between* reads. Its design note says the renderer never aborts on its own and relies on the caller's currency check, but a pending read is exactly where that check cannot run. When a zoom supersedes a 40–100% whole-frame stream — the range where the frame exceeds the chunk budget and takes the streaming route — the abandoned body keeps its HTTP/1.1 connection. Six abandoned streams exhaust Chromium's six-connections-per-origin pool, after which every request, edit commands included, queues with no error: the frozen sliders and idle CPU. The six parked connections match the six superseded zooms the owner performed.

Fix (`frontend/webgpu-preview.js`): `readSourceChunk()` races the pending read against a 200 ms currency tick, so a superseded stream cancels its reader — releasing the socket — and throws the superseded error the failure policy already understands; the `finally` also cancels when the stream exits without completing, covering the failed-mid-stream paths. Cancellation is guarded for readers that do not implement it (the test fakes and older runtimes), and the `source-transport` test sandbox now supplies the timers the race needs.

Evidence: `tests/transport-pool-exhaustion.js` fires 12 superseding zooms at 42 MP and requires a cheap same-origin request to answer and the viewer to converge. Development tree `output/performance/transport-pool-exhaustion-dev.json`: probe 200 in 4 ms, converged in 2.5 s to exact at 6534. Packaged (rebuilt PyInstaller + `pack:dir`) `transport-pool-exhaustion-packaged.json`: probe 200 in 4 ms, converged in 3.05 s, zero page errors. The pre-fix behaviour is the live capture recorded above and preserved in `output/performance/live-window-inspection.json`, `live-window-inspection-2.json` and `live-window.png`. `node --test` 237 pass with the change; the owner's app was relaunched from the rebuilt package.

### 15.56 Second connection-pool leak at native edges — pre-reader abandonment — 2026-09-24

Symptom (owner, live packaged session on the build containing §15.53 and §15.55): after zooming in and out the viewer stuck on "Preparing view — Native region exact" (the dock read "Updating — Native region exact" at capture time), 183%; sliders and controls sometimes still responded.

Live capture (attached over the §15.54 diagnostic port; `output/performance/live-window-2026-09-24T14-27-18-217Z.json` + `.png`). Measured: the viewer was `updating` at the native tier with the accepted exact 7968 frame one generation old (`previewGeneration.hdr` 42, accepted generation 1); `gpuDraftInFlight` held coordinator token 21 (`refinement` / `zoom-scale`, **age 256.6 s**) with token 42 pending behind it for the last 41.9 s; scheduler metrics `inputCount 42 / frameCount 1 / coalescedFrames 40` (the one frame took **17.96 s**); every app process and the GPU/network process measured **0.000 s CPU over 8 s** (backend 0.031 s), `py-spy` showed every backend thread idle, and `device.queue.onSubmittedWorkDone()` drained immediately (so nothing was parked on the GPU); netstat showed **six established renderer→backend connections** created 14:22:13–14:22:58Z, before token 21 dispatched at 14:23:02Z; a cheap `fetch('/api/session/{id}')` from the page **did not answer within 4.5 s** (twice). `state.gpuPreview.proxyInflight` held exactly one key — the stuck pass's whole-frame load (`:hdr:6051:…:source`) — so the six sockets did not belong to any load the app still tracked. Rescue probe (`output/performance/live-stall-rescue.json`): `state.gpuPreview.sourceAbort.abort()` released the sockets immediately (netstat 12 → 4 lines), the next pool probe answered in **4 ms**, the coordinator settled and the viewer converged to ready/exact 7968 in **28.4 s**. The sockets were therefore source fetches held by the session-scoped controller, abandoned where neither the coordinator token nor the §15.53 watchdog nor the §15.55 reader tick could reach them. Inferred from the reproduction rather than resolvable in hindsight, because a body parked before its reader exists never produces a resource-timing entry: those six were pre-reader abandonments of the kind below.

Cause, proven by reproduction: in `loadProxyStreaming` the `await fetch()` resolves with a successful **streaming** response, and every check before a reader exists — geometry mismatch (`Stale WebGPU geometry proxy rejected`), the pre-upload supersession check, texture-admission failure — throws with the body unread. Nothing consumes or cancels it afterwards, so the HTTP/1.1 connection stays claimed while the backend keeps writing into backpressure. Native edges widen the window: response headers wait on the cold source-mip build (the live session's `source-mip-progress` polls ran ~31 s), so a supersession arriving while headers are pending always loses that race. `tests/native-region-stall.js` makes it deterministic — 42.4 MP fixture (7968 × 5320), §8 reference window 2560 × 1440, eight zoom pairs whose native pass is superseded 150 ms after dispatch while streamed source headers are withheld 500 ms (the same effect as the cold build; `--header-delay-ms`). Pre-fix, this wedges at **pair 5 in three consecutive runs**: the pool probe times out, four `proxy-stream/hdr` 200-bodies sit with no reader, no cancel and no abort, `gpuDraftInFlight` stays set on a ~1.0 s-old `zoom-scale` token, and six connections are held — driver exit 1, retained as `output/performance/native-region-stall-nofix.json` + `.png`.

Fix (root cause, `frontend/webgpu-preview.js`): the whole post-`fetch` body of `loadProxyStreaming` now lives in one try/catch/finally that owns the response; every exit that did not consume it cancels the reader (mid-stream paths) or the response body itself (the pre-reader throws), so an abandoned stream always gives its connection back. This is not a widening of the §15.53 watchdog — its in-flight guard is correct and untouched — and not a reconsideration of §15.55: the reader tick already frees a stream that has a reader, and this closes the window before one exists. Named follow-up, not required by this evidence: pass the coordinator token's signal into the source options so a supersession aborts a request whose headers have not arrived instead of cancelling on header arrival. After the fix the same driver passes: no wedge, **zero** undrained bodies (six stream-level cancellations were reported by the run but not serialized into the retained JSON — see the review follow-up below, which records them), pool probe **200 in 3 ms**, two connections held, converged, exit 0 (`output/performance/native-region-stall.json` + `.png`).

Control and regression: the injection itself is the pre-fix failure — the retained no-fix records carry no reader, cancel or abort marks, so the same assertions fail on them. Commands: `node tests/run-in-electron.js tests/native-region-stall.js --packaged` with `HDR_FINISHER_ELECTRON_WINDOW_SIZE=2560x1440`, after `& '.\.venv\Scripts\python.exe' -m PyInstaller --noconfirm .\HDRFinisherBackend.spec` and `npm run pack:dir`. The two earlier drivers still pass on the rebuilt package, measured: `transport-pool-exhaustion.js` probe 200 in 81 ms, convergence 2.55 s; `zoom-refinement-stall.js` burst converged in 1.52 s with zero re-arms, and the injected dropped wake-up still fails to converge with the watchdog disabled (6.1 s) and converges with it enabled (1.53 s, one re-arm). `node --test`: **236 pass, 0 fail** (9.4 s; §15.55 recorded 237 — today's auto-discovered suite counts 236, with zero failures either way). The live-diagnostic probes were removed; `tests/native-region-stall.js` stays as the regression driver. The owner's app was relaunched from the rebuilt package.

**Review follow-up (2026-09-24).** Corrections to the record above, measured against the retained JSONs: the pre-fix run was a *transient* starvation, not a permanent wedge — its 800 ms monitor probe missed at pair 5, but the final 5 s probe answered 200 in 1513 ms and the viewer converged in 515 ms (`native-region-stall-nofix.json`); only one of the "three consecutive" pre-fix runs is retained. Code changes (`frontend/webgpu-preview.js`, uncommitted): (1) `loadProxyStreaming` drains its staging ring *before* registering the proxy and recording metrics, restoring the pre-§15.56 meaning of streamed `totalMs`/`durationMs` (which include the final queue drain, as the tiled and region routes do) and ensuring a late throw can no longer destroy a texture already in `this.proxies`; the 409/507 path marks its already-read body complete. (2) Same leak class outside the source routes: `loadMaskLeaf` returned on a refused or geometry-mismatched 200 without reading a native-size mask body — it now cancels the body; the mask-tile batch cancels its body when `maskTileBatch` is absent. (3) The 1×1 probes in `loadProxyStreamed`/`loadProxyRegion` drain before their currency check (consistency; a 1×1 body was not a practical leak). Driver hardening (`tests/native-region-stall.js`): a body counts as undrained only 250 ms after the application actually received it (post-injection `deliveredAt`), removing the transient `und1` samples of the earlier post-fix run; rejected `json()`/`arrayBuffer()` reads count as consumed; a wedge requires a second 5 s probe miss after the 800 ms one; and a coverage assertion (`--min-abandons`, default 4) fails a run in which the injection abandoned too few source responses before a reader existed, so a slow machine cannot pass vacuously. Measured on the dev tree (Electron, 2560 × 1440; the packaged build was not rebuilt because the owner's packaged app was running): fixed — 6 pre-reader abandonments, all 6 cancelled, 0 undrained, probe 200 in 2 ms, converged, exit 0 (`native-region-stall-dev.json`); control with the pre-reader `response.body.cancel()` disabled — 6 abandonments, 0 cancelled, 5 undrained, wedge at pair 7 surviving the 5 s re-probe, **not converged**, exit 1 (`native-region-stall-control.json`), fix restored byte-identical afterwards. `transport-pool-exhaustion.js` probe 200 in 6 ms, converged 2.05 s; `zoom-refinement-stall.js` burst 1.52 s / 0 re-arms, injected stall 6.1 s without watchdog vs 1.53 s / 1 re-arm with it; `node --test` 236 pass / 0 fail. The named follow-up (token signal into source options) stays deferred; if taken, it needs a refcounted signal per `proxyInflight` key (a second token joins the first's promise at the dedupe), an `AbortError` → `supersededSourceError` mapping (the failure policy otherwise records supersession as a transport failure), and composition with the session-scoped `sourceAbort`. Still owed: re-run the three drivers `--packaged` after the next PyInstaller + `pack:dir`.

**Working tree.** All changes are uncommitted, as the increment policy requires; evidence JSONs live under `output/performance/` (gitignored). No commit was made.
...
Symptom (owner): the zebra overlay did not update live with exposure adjustments. Reproduced (`tests/zebra-overlay-live.js`, new; 4000 × 2667 noisy fixture, zebra threshold 400 nit so the zebra edge falls inside the slider's range, HDR Exposure dragged from 0 EV to the latitude-preset floor of −1.95 EV): with the fix disabled, **zero** overlay requests were made during or after the drag and the overlay on screen stayed the revision-1 render (coverage 0.729) while the edit reached revision 22 (`output/performance/zebra-overlay-live-control.json`). So the overlay was not merely "not live" — it was never refreshed after the drag at all.

Cause: the overlay is a backend-rendered PNG composited over the GPU preview and only changes when re-requested. Its sole refresh after an edit was in `settlePreview` after a GPU render; the refinement pass (`refinePreview`) presents the exact frame without refreshing it, and `settlePreview` then takes its "already exact" early return, so the overlay kept the pre-drag frame indefinitely.

Fix (`frontend/app.js`, uncommitted): the scheduler's scope pass — which already runs throttled and latest-wins during a drag, after settle and after refinement — now also calls `requestLiveOverlay(tier)`, a loop with one overlay request in flight and the newest tier pending (so a drag produces a steady stream instead of aborting each request, and the settled request always runs). Interim (interactive-tier) overlays are rendered at `min(interactive, settled)` edge and may lag the picture by their own render time but never go backwards, change lane/geometry, or survive the overlay being switched off; a refused interim request keeps the previous overlay instead of blanking it. Settled overlays keep the exact currency check (revision and presented generation). `state.overlayPresented` records what the overlay on screen was rendered for, and a settled request already satisfied by the settle path is skipped. Measured (dev tree, Electron): zebra coverage **0.729 → 0.655 → 0.289 → 0** during the drag, final overlay a settled render of the final revision/generation, exit 0 (`zebra-overlay-live.json`); false-colour variant follows likewise (`false-color-overlay-live.json`). Cost: 48 interactive frames during a 2.4 s drag with the overlay on and off alike (`overlay-live-{off,zebra}.json`, two runs each). Stall drivers after the change: `transport-pool-exhaustion` probe 5 ms / converged 2.03 s; `zoom-refinement-stall` burst 2.55 s / 0 re-arms (1.52 s earlier today; the overlay is off in that driver, so read as run-to-run variance, not attributed), injected stall 6.2 s without watchdog vs 1.53 s / 1 re-arm with it; `native-region-stall` 4/4 pre-reader cancellations, converged. `node --test` 236 pass / 0 fail. User guide: `docs/user-guide/viewer-and-analysis.md` (Zebras) states the live/settled behaviour.
