...
- geometry/perspective transform performance: crop, straighten, and perspective (including the new guided Rotate axis) are baked into the WebGPU proxy on the server rather than run in the shader, so every interactive drag tick round-trips to Python/Pillow instead of getting GPU-speed feedback like tone/color/local adjustments do. Profiled on a ~1024px interactive proxy: ~120-130ms per unique keystone value (~39ms pure-Python safe-rectangle search + ~76ms Pillow 4-channel `PERSPECTIVE`+`BICUBIC` warp), vs. ~37ms for a pure-rotate-only change or a cached-identical value. Two candidate levers, not yet implemented: (1) vectorize `_perspective_safe_rectangle`'s pixel-loop search in numpy (contained, low-risk); (2) use a cheaper resample filter for interactive-tier draft previews only, keeping `BICUBIC` for committed/export renders (bigger win, higher risk since `apply_geometry` is called from 7+ sites including final export and local-adjustment mask baking, so the draft/final distinction must be threaded through carefully).
