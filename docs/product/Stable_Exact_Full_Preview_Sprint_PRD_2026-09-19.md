...
**BLOCKER — fix before anything else:** `gpu-highlight-compression-parity` regressed during this sprint and is now flaky. Six runs each on the same adapter and server: pre-sprint `83bca70` passes 6/6; the Phase 4 checkpoint `ecb3c4a` passes 3/6; with tiled execution it passes 1/6. Every numeric assertion still passes to five decimal places — the failure is `replacementCurrent`, because `renderGpuDraft` returns `false` for the replacement render.

The mechanism is captured in the Phase 4 tiled-execution evidence. Phase 1 redefined `previewNeedsRefinement()` as `!selectedTierReady()`; the suite renders at `longEdge: 512` while the tier is 1K, so the accepted presentation is never exact, so the scheduler arms a refinement pass that pre-sprint code disarmed. That background render increments `state.gpuRenderSerial` and the suite's concurrent render loses the race. This is a Phase 1 lifecycle fix, not a tiled-execution fix. Decide between stopping refinement from arming when the shortfall came from an explicitly requested lower edge, and guarding `refinePreview` against racing an in-flight external render.
