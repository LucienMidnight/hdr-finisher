...
**The decision Phase 7's scope work is waiting on.** The scope pass is a downsample, not a histogram: each cell carries the mean and peak over its footprint of the finished frame, and Direct reduces on demand from a resident whole-resolution `finishTexture` at whatever resolution the UI asks for. Tiled cannot keep that texture. An exact tiled accumulation is available — each tile writes partial sums divided by the full cell count, additive blending on RGB and max blending on alpha — but accumulation happens at encode time at one fixed resolution while `analyzeScope` is called later at a resolution the UI chooses. Either the accumulation runs at the maximum requestable resolution and `analyzeScope` box-reduces from it, exact only when the requested grid divides it, or the scope resolution becomes a property of the generation and the UI takes what it is given. That is a decision about what a scope promises, so it is written down rather than guessed at. Until it is taken the Phase 4 behaviour stands and is truthful: a tiled presentation drops the GPU scope source and settles through the CPU route.

**Next safe edits, in the order they unblock each other:** (1) take the scope decision above and implement it; (2) comparison and A/B tier and generation disclosure, and proofing; (3) spatial film effects on the CPU strip path; (4) the Phase 6 loose end, the whole-frame resolved denoise proxy, which is now the tile scheduler's *only* remaining refusal.
...
- `codebase/.venv/Scripts/python.exe -m pytest -q`, **run from `codebase/`** — running it from the repository root collects `codebase/output/libultrahdr-build/.../googletest` and fails at collection with 8 errors. Result after Phase 7's three commits: `1255 passed, 3 skipped` (it was `1254 passed, 3 skipped` at Phase 6).
...
**Active failure or unresolved decision:** no failures. One open decision, and it is a product decision rather than an engineering one: the tiled scope resolution contract described under *Phase 7 so far* above. It blocks Phase 7's scope work and nothing else.
