...
The hands-on run exposed four Linux integration follow-ups:
...
4. **Polish Linux HDR preview gradients with presentation-only dithering.** A v0.8.8 hands-on comparison found more visible contouring in smooth gradients on KDE/Wayland with NVIDIA than on the same content under Windows and macOS. The Technical scope still reported a qualified native-Wayland HDR presentation, a 16-bit-float `rgba16float` WebGPU canvas, 10-bit components, and 30-bit screen output; the result was unchanged when the Xiaomi HDR display was reduced from 143.98 Hz to 59.95 Hz. Treat this as deferred preview polish rather than an export-correctness blocker. Investigate subtle, stable blue-noise or equivalent dithering at the final Linux HDR presentation boundary, calibrated to the detected output depth. It must not alter authoritative processing, scopes, proofs, or exported pixels, and it must avoid temporal shimmer, patterned noise, and visible grain in flat fields. Validate with neutral and colored ramps on the qualified NVIDIA/KWin path and compare against Windows and macOS before enabling it by default.
...
- **Camera-linear RAW import bridge sprint (recorded and amended 2026-09-03):** the ordinary mosaiced RAW route must stop asking LibRaw to combine as-shot white balance, ACES conversion, highlight clipping, and a bounded 16-bit output in one `postprocess()` call. `2L1B8631.CR2` demonstrates that this ordering can create a broad blue/cyan ceiling even though only four visible-mosaic samples reach LibRaw's global white level. The approved bridge retains LibRaw/rawpy and AHD, requests unity-WB/no-auto-scale camera RGB, crosses into float32 immediately, and performs as-shot WB plus camera-to-ACEScg conversion without clipping. Product-owner testing then established that honest unclamped camera RGB alone exposes genuinely channel-clipped magenta in Niagara and both Sony fixtures, matching darktable with its highlight module bypassed. The approved iterative amendment adds one default-on, bypassable NumPy `opposed_color_v1` CFA reconstruction before AHD, with a single clipping-threshold control and stable recipe/version metadata. It does not switch globally to LibRaw `Blend` or add a second reconstruction engine. The sprint contract, private cross-vendor corpus, fallback rules, and acceptance gates are defined in [Camera-Linear RAW Import Bridge Sprint](Camera_Linear_RAW_Import_Bridge_Sprint_PRD_2026-09-03.md).
...
- **RAW highlight reconstruction (implemented 2026-09-03; iterative scope):** qualifying ordinary mosaiced RGB/RGBG Bayer and X-Trans RAWs expose a **Highlight Reconstruction** module immediately after RAW DEVELOPMENT. Its bypass eye is the Off control; the sole v1 method is `opposed_color_v1`, and threshold changes trigger transactional re-development. New imports enable it at `1.0`; pre-module v4 projects migrate to an explicit bypass. The NumPy implementation reconstructs only clipped CFA photosites before AHD, leaves sub-clipped codes untouched, records per-channel diagnostics, and preserves preview/export parity through the canonical source array. Segmentation-based and guided-laplacian methods remain deferred until representative failures justify their complexity. Do not treat a global switch to LibRaw `ReconstructDefault` or `Blend` as an accepted alternative.
...
#### Long-term RAW engine options to explore (recorded 2026-09-03)

LibRaw remains the preferred near- and medium-term proprietary RAW decoder and metadata foundation. It is actively maintained for new cameras, formats, metadata, and decoder hardening, and is used by projects including ImageMagick, OpenImageIO, digiKam, and KDE image-format plugins. However, LibRaw explicitly describes its dcraw-derived RAW-to-RGB conversion as basic compatibility functionality rather than a production-quality renderer and does not plan major further development of that rendering layer. HDR Finisher should therefore depend on LibRaw for unpacking, not make its complete `postprocess()` renderer a permanent color/highlight boundary.

Any long-term project should separate these replaceable responsibilities:

```text
RawDecoder
    -> normalized float32 RawSensorFrame + metadata + clipping masks
    -> Demosaicer
    -> float32 camera RGB
    -> HDR Finisher-owned WB, camera color, ACEScg, and highlight stages
```

The proposed `RawSensorFrame` contract should include the visible mosaic, CFA pattern/color description, active area and crop, orientation, per-channel black and white levels, as-shot neutral/WB, camera matrices/profile identity, genuine sensor-clipping masks, decoder/version provenance, and any mandatory DNG operations. It must preserve negative and greater-than-one downstream values and must not encode scene-linear data through a bounded display representation.

Options to evaluate in a separately approved sprint:

1. **LibRaw + librtprocess — leading candidate.** Keep LibRaw for broad file decoding and use librtprocess's float-input Bayer/X-Trans algorithms (including AMaZE, RCD, AHD, DCB, LMMSE, and Markesteijn) plus optional highlight recovery. This aligns directly with the desired float32 boundary. librtprocess is GPLv3, which is compatible with HDR Finisher's existing GPLv3 distribution, but it adds a native C++ build/binding and cross-platform packaging obligation.
2. **RawSpeed as an alternate decoder.** RawSpeed can supply unmodified/scaled integer RAW or normalized float sensor data, CFA layout, and black levels. It deliberately does not demosaic, apply WB/color correction, or produce a viewable image. Define the decoder interface so RawSpeed can be evaluated later, but do not carry two decoder stacks until camera-coverage, performance, security, or maintenance evidence justifies the cost.
3. **Colour-Demosaicing as a prototype/reference.** Its BSD-licensed Python implementations of Bilinear, Malvar, and Menon demosaicing are useful for deterministic experiments and algorithm tests. Its Bayer-only scope, smaller algorithm set, and Python/SciPy performance make it a reference or fallback candidate rather than the current production favorite.
4. **darktable and RawTherapee as quality oracles.** Their full pipelines and real-image behavior should inform validation, but embedding either application wholesale would import a large, coupled processing/configuration surface. Prefer reusable components and compare outputs rather than shelling out as the primary production architecture.
5. **Rawler/DNGLab as an emerging Rust option.** Track its camera/format coverage and safety model, but its maintainers currently describe DNGLab as alpha and Rawler's API as unstable. It is a watchlist item, not a replacement commitment.

The long-term qualification must compare candidate demosaicers on Bayer and X-Trans detail, false color, moire, noise, genuinely clipped highlights, saturated non-neutral subjects, bad pixels, orientation/crop behavior, memory, cancellation, and packaged Windows/macOS/Linux performance. It must also decide camera-matrix versus DCP profile support, per-channel clipping semantics, CFA-domain defect repair, and whether highlight reconstruction occurs before or after demosaic. The private corpus under `D:\Photos\HDR Test Images\Test Suite Images` is the initial real-media matrix; committed tests must remain synthetic or redistributable.

Do not begin this larger replacement inside the camera-linear bridge sprint. The bridge establishes the float-friendly seam and fixes the present clipping-order defect; the later project may replace the decoder or demosaic component independently.

Primary upstream references: [LibRaw scope and maintenance policy](https://github.com/LibRaw/LibRaw), [librtprocess](https://github.com/CarVac/librtprocess), [RawSpeed](https://github.com/darktable-org/rawspeed), [Colour-Demosaicing](https://github.com/colour-science/colour-demosaicing), and [DNGLab/Rawler](https://github.com/dnglab/dnglab).
