...
*August 22, 2026 implementation addendum: a real Sony RAW project exposed Safari-visible patchy noise in JPEG Ultra HDR while Chrome showed milder noise. The file was structurally valid, but libultrahdr had mapped an 8-bit gain image across `-14.3` to `+7.29` stops even though the authored display capacity was only `+2.318` stops; near-zero RGB channel ratios therefore consumed most available codes. JPEG Ultra HDR export now applies an automatic publishing-only content-boost guardrail of `-4` to `+4` stops (`0.0625x` to `16x`), expanding the positive limit whenever the measured authored HDR peak requires more headroom. This retains three-channel color separation and leaves the rendered SDR/HDR endpoints unchanged while clipping only more-extreme per-channel ratios. AVIF receives no duplicate clamp because bundled libavif already removes the outer 0.1% of ratio outliers when selecting its 10-bit per-channel map range. Direct-PQ JPEG XL remains outside all gain-map logic. Unit, real libultrahdr encode/probe/decode, exporter, proofing, import, SDR, frontend-contract, and launcher coverage passes. A matched test export validates the new `-4` / `+4` metadata and awaits physical Safari/Chrome visual acceptance; full-resolution quality-100 gain maps may grow materially because the restored code range contains more spatial information, so Web Default or Web Optimized remains preferred for routine publishing.*
