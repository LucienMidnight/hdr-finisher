...
- The cross-platform icon-library migration is incomplete. Grade and Metadata disclosure chevrons still reference private-use Segoe Fluent Icons glyphs (including `E76C`) from CSS, so macOS displays missing-glyph squares; creating a packaged `.app` will not correct them. Before the next macOS package, audit every remaining `Segoe Fluent Icons` / `Segoe MDL2 Assets` dependency and replace private-use glyphs with bundled library assets (prefer the existing Tabler icons where available), then validate the same icon states in both the Windows and macOS desktop applications.
