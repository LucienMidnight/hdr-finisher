...
Eraser strokes are not ordinary chronological paint strokes. They form a destructive attenuation layer that is always evaluated above the completed positive Brush mask. The authoritative CPU mask and the browser fallback overlay must use the same sequence:

1. Accumulate all non-erase Brush strokes into the positive paint mask. Brush Flow builds coverage and per-stroke Density/opacity limits the stroke contribution.
2. Apply whole-mask **Shift Edge** to the positive mask.
3. Apply whole-mask **Feather**.
4. Apply expression **Invert**, when enabled.
5. Apply whole-mask **Mask Opacity**.
6. Accumulate all erase-stroke attenuation and multiply it into the result as the final mask operation.
7. Clamp the finished mask, apply the local HDR or SDR grade through it, then apply the local adjustment's overall opacity.
8. Apply **Bypass** as the final gate over the entire local adjustment. Bypass disables the complete local grade, not merely a control group or mask stage. The mask overlay may remain visible for inspection while the grade is bypassed.

In compact form, for positive mask `P`, shared mask processing `S`, mask opacity `O`, and erase strengths `E_i`:

`M_final = clamp(S(P) * O * product(1 - E_i), 0, 1)`

`S(P)` includes Shift Edge, Feather, and any requested inversion. This ordering is intentional even when an erase stroke appears earlier than a later paint stroke in the serialized stroke list: erase remains on top. It is what allows the eraser to remove coverage generated outside the original paint footprint by positive Shift Edge, Feather, or Invert. Erasing must never increase mask coverage at any pixel.
