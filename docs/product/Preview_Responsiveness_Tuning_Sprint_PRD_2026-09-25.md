...
**Packaged build (Section 8).** Rebuilt from `a73284b` (includes every P1–P7 change): PyInstaller backend (`HDRFinisherBackend.spec`) plus `npm run pack:dir` into `dist-electron/win-unpacked`. The backend bundle was built from the `.venv` reinstalled from `requirements-dev.txt` on 2026-09-25, after a cleanup mistake deleted part of it; packages are the newest versions `requirements.txt` allows (for example numpy 2.5.3, Pillow 11.3.0), so they may differ from earlier bundles. Every run below used `--packaged` at 2560×1440 on the reference machine, one at a time. All 13 passed: `electron-smoke`, release suite (checkbox off and on), full `headline-latency`, `drag-gpu-load`, `luma-feather-latency`, `luma-feather-quality`, `highlight-ceiling`, `highlight-anchor-stability`, `budget-route`, `pixel-magnification`, `technical-panel-fit`, and `desktop/tests/preview-menu.js` (needs a packaged executable; not run before).
