...
| Local adjustments and projects | `test_local_adjustments.py`, `local-adjustments-interaction.js` | Soft mask algebra, EV luminance trapezoids, fixed-source selection, HDR/SDR grade independence, tile determinism, r8 preview quantization, compiled-mask reuse, geometry mapping, revision conflicts, undo, project round trips, persistent stack presentation, visible on-image gizmos and gestures for every core mask, brush-only Erase eligibility, in-flow group expansion, HDR/SDR folder-tab interaction, and nonblank histogram/waveform refresh after local edits |
