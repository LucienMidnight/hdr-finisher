...
### A note on the linear DNG export, for whenever DNG input is built

PhotoLab’s **linear DNG** export (demosaiced, denoised, lens-corrected, but with no baked tone curve — “tonal and color corrections are to be done in your main photo editing software”) is a plausible match for the “already-rendered linear DNG” input candidate scoped in [HDR_Finisher_PRD_v1.2.md §4](../product/HDR_Finisher_PRD_v1.2.md) and detailed further in [Codebase_Review_Cleanup_and_Round_Trip_Results_2026-08-11.md](../testing/Codebase_Review_Cleanup_and_Round_Trip_Results_2026-08-11.md#dng-recommendation-later-constrained-candidate-only): no tone curve applied means the samples stay scene-linear with real headroom, and the mandatory DNG tags (`ColorMatrix1/2`, `CalibrationIlluminant`, `AsShotNeutral`, black/white levels, `BaselineExposure`) should give a deterministic mapping to ACEScg without needing camera-profile selection or general RAW development. This is unverified: confirm with `exiftool`/`dcraw -i -v` on a real exported file that DxO isn’t silently baking a display tone curve in anyway, and that those tags are populated, before treating it as a real candidate. See the open [DNG research questions](../testing/Codebase_Review_Cleanup_and_Round_Trip_Sprint.md#55-dng-research-questions) for the full evaluation checklist.
