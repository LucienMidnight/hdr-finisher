...
Implementation note, August 25, 2026: the current branch adds lane-specific, schema-versioned denoise settings; the four reconstruction-only controls; locked **Photo / Fine**, **Photo / Mixed**, **Render / Fine**, and **Render / Coarse** methods; a **Custom** editor for one through four wavelet scales, threshold, and scene-linear luma/chroma noise levels; explicit Recalculate; A/B; and Off, Preparing, Ready, Dirty, Recalculating, and recoverable Error states. Two-scale methods keep the direct resolve path. One-, three-, and four-scale methods use the generic decimated reconstruction pipeline with persistent scratch textures and parameter buffers allocated with the analysis cache, so live drags perform no allocation or analysis. These starting method values are provisional engineering defaults; Phase 4 corpus work must tune or reject them before product acceptance. The active 1K/2K/4K tier is pinned while its original/resolved pair is retained, and switching to an inactive lane evicts the previous lane's coefficient/resolved cache. Source and packaged ORF UI runs pass at every proxy tier, including 200% zoom/scroll stability, rapid reversal, A/B, preset/recalculate, Exposure isolation, lane eviction/restoration, and retained-original disable. A source-build WebGPU run also exercises four-scale analysis, persistent-scratch reconstruction, live resolve reversal, unrelated Exposure isolation, and resource eviction. Backend tests cover schema validation, undo/redo, project round-trip, and RAW-redevelopment preservation. This phase is implemented for evaluation but is not accepted ahead of the unresolved Phase 2 memory stop gate.

An aggressive **Photo / Mixed** manual test at 4K and 659% zoom exposed visible 8x8 tiling in smooth skin when Amount, Luminance, and Color Noise approached 100%. The cause was equal reconstruction strength at every decimated Haar level, which allowed coarse photographic structure to be removed like fine noise. CPU and WebGPU reconstruction now use the same conservative per-level weights (`1.0`, `0.55`, `0.25`, `0.1` from fine to coarse). A maximum-strength three-level regression preserves at least 65% of a deterministic third-level structural pattern, preventing coarse structure from collapsing into block averages while retaining full fine-scale control.
