...
Start with **Filmic**. Adjust Base Rendition contrast/skew, Exposure, Highlight Recovery, and Shadow until the image works as an ordinary SDR photograph. Leave **Match HDR color** enabled unless the fallback genuinely needs a separate color treatment.
