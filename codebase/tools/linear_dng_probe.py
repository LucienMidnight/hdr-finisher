...
def _tag_value(page: Any, name: str, default: Any = None) -> Any:
...
def build_bounded_preview(path: Path, output: Path, *, max_long_edge: int = 2400) -> dict[str, Any]:
    ...
    with tifffile.TiffFile(path) as tif:
        ...
        if not page.is_memmappable:
            raise RuntimeError("Bounded preview currently requires a memory-mappable DNG page.")
        reasons, _notes, _metadata = qualify_page(
            page, int(page.compression) in tifffile.TIFF.DECOMPRESSORS
        )
        if reasons:
            raise RuntimeError("DNG does not qualify: " + " ".join(reasons))

        mapped = tifffile.memmap(path, mode="r")
        factor = max(1, int(np.ceil(max(mapped.shape[:2]) / max(1, max_long_edge))))
        output_height = max(1, mapped.shape[0] // factor)
        output_width = max(1, mapped.shape[1] // factor)
        ...
        for output_row in range(output_height):
            ...
            block = np.asarray(mapped[start : start + factor, :cropped_width, :3], dtype=np.float32)
        ...
        black = _rational_array(_tag_value(page, "BlackLevel", (0,) * 3), 3).astype(np.float32)
        white = _rational_array(
            _tag_value(page, "WhiteLevel", ((1 << bits) - 1,) * 3), 3
        ).astype(np.float32)
        camera_rgb = (camera_rgb - black) / np.maximum(white - black, np.float32(1e-12))

        analog_balance = _rational_array(_tag_value(page, "AnalogBalance", (1,) * 3), 3)
        camera_calibration = _rational_array(
            _tag_value(page, "CameraCalibration1", tuple(np.eye(3).reshape(-1))), 9
        ).reshape(3, 3)
        camera_neutral = _rational_array(_tag_value(page, "AsShotNeutral"), 3)
        forward_matrix = _rational_array(_tag_value(page, "ForwardMatrix1"), 9).reshape(3, 3)
...
def qualify_page(page: Any, decoder_available: bool) -> tuple[list[str], list[str], dict[str, Any]]:
    reasons: list[str] = []
    notes: list[str] = []
    ...
    for name in required_tags:
        if page.tags.get(name) is None:
            reasons.append(f"Required color metadata {name} is absent.")

    present_render_tags = [name for name in UNIMPLEMENTED_RENDER_TAGS if page.tags.get(name) is not None]
    if present_render_tags:
        reasons.append("Unimplemented rendering metadata is present: " + ", ".join(present_render_tags) + ".")

    defaults = {
        "BlackLevel": _tag_value(page, "BlackLevel", [0.0] * samples),
        "WhiteLevel": _tag_value(
            page,
            "WhiteLevel",
            [1.0] * samples if dtype.kind == "f" else [float((1 << bits[0]) - 1)] * samples,
        ),
        "AnalogBalance": _tag_value(page, "AnalogBalance", [1.0] * samples),
        "BaselineExposure": _tag_value(page, "BaselineExposure", 0.0),
        "ActiveArea": _tag_value(page, "ActiveArea", [0, 0, int(page.imagelength), int(page.imagewidth)]),
        "DefaultCropOrigin": _tag_value(page, "DefaultCropOrigin", [0.0, 0.0]),
        "DefaultCropSize": _tag_value(
            page, "DefaultCropSize", [float(page.imagewidth), float(page.imagelength)]
        ),
        "Orientation": _tag_value(page, "Orientation", 1),
    }
    ...
    metadata = {name: _json_value(_tag_value(page, name)) for name in metadata_names}
    metadata["resolved_defaults"] = {name: _json_value(value) for name, value in defaults.items()}
    return reasons, notes, metadata


def probe(path: Path, *, existing_session_bytes: int = 0, sparse_decode: bool = True) -> dict[str, Any]:
    try:
        import tifffile
    except ImportError as exc:
    ...
    with tifffile.TiffFile(path) as tif:
        ...
        reasons, notes, metadata = qualify_page(page, decoder_available)
        ...
        if sparse_decode and memmappable:
            mapped = tifffile.memmap(path, mode="r")
            row_step = max(1, mapped.shape[0] // 8)
            column_step = max(1, mapped.shape[1] // 16)
            sample = mapped[::row_step, ::column_step]
            axes = tuple(range(sample.ndim - 1)) if sample.ndim > 2 else tuple(range(sample.ndim))
            decode = {
                "attempted": True,
                "method": "read-only memory map with sparse sample",
...
def main() -> int:
    ...
    if args.preview_output is not None and result["qualifies"]:
