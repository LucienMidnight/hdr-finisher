...
def test_jxl_and_ultrahdr_exports_are_capability_gated() -> None:
    ...
    assert "ultrahdr_decoder" in capabilities
    assert "avif_gain_map_decoder" in capabilities
    ...
    assert capabilities["ultrahdr_decoder"].status == capabilities["ultrahdr_encoder"].status
