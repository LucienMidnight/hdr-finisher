...
def apply_gain_map(
    ...
    *,
    ...
    if gain_map.map_planes not in {1, area.planes}:
        raise DngOpcodeError("GainMap map plane count must be one or match AreaSpec planes")
    ...
    for start in range(area.top, area.bottom, rows):
        ...
        # DNG map origin/spacing are expressed in normalized image coordinates.
        gv = (yy / max(height - 1, 1) - gain_map.map_origin_v) / gain_map.map_spacing_v
        gh = (xx / max(width - 1, 1) - gain_map.map_origin_h) / gain_map.map_spacing_h
        ...
        for relative_plane in range(area.planes):
            map_plane = 0 if gain_map.map_planes == 1 else relative_plane
    ...
    *,
    ...
    for start in range(0, height, rows):
        ...
        for channel in range(channels):
            ...
            output[start:end, :, channel] = _sample_bilinear_zero_border(
...
def _sample_bilinear_zero_border(
