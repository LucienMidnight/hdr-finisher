...
The repository is no longer at the original vertical-slice stage. It now provides a usable single-image HDR finishing workflow with real AVIF gain-map export, true HDR browser preview, Apple HEIC HDR reconstruction, independent SDR fallback control, histogram scopes, and a functioning RGB curves editor.
...
- Automated tests for core math, classification behavior, HEIC reconstruction, preview/export helpers, and API flow
...
- `pytest -q` passes with `24 passed`
...
- False color / zebra overlay is not implemented yet
...
- **UI state reliability:** the earlier broken-image / empty-state overlap bug has been cleaned up during the true-preview and UI passes.
...
1. Implement false color / zebra overlay as the next remaining PRD-aligned viewport aid
2. Decide whether JPEG Ultra HDR export should move ahead before JPEG XL, based on actual 2026 delivery priorities
3. Add packaging work for Windows first, including binary bundling and clean-machine validation
4. Continue tuning preview quality/performance tradeoffs once the remaining feature gaps are closed
